...
import { MapPin, Utensils, Train, Camera, Bed, ShoppingBag, Info, Navigation, Sparkles, BookOpen, Map, X, ZoomIn } from 'lucide-react';
...
const ActivityItem: React.FC<ActivityItemProps> = ({ item, isLast }) => {
  ...
  const getIcon = (type: ActivityType) => {
    switch (type) {
      case ActivityType.FOOD: return <Utensils size={16} className="text-japan-red" />;
      case ActivityType.TRANSPORT: return <Train size={16} className="text-indigo-japan" />;
      case ActivityType.SIGHTSEEING: return <Camera size={16} className="text-matcha-dark" />;
      case ActivityType.STAY: return <Bed size={16} className="text-gray-600" />;
      case ActivityType.SHOPPING: return <ShoppingBag size={16} className="text-sakura-dark" />;
  ...
  const displayImage = imgError ? './assets/pictures/placeholder.jpg' : item.imageUrl;
    ...
    <>
      <div className="flex group mb-8 last:mb-0">
        ...
        <div className="flex flex-col items-center mr-4 w-12 flex-shrink-0 pt-2">
          <span className="text-xs font-bold text-gray-400 font-mono tracking-tighter">{item.time}</span>
          <div className={`w-0.5 h-full my-2 bg-gray-200 rounded-full ${isLast ? 'hidden' : 'block'}`}></div>
        </div>
            ...
            {item.imageUrl && (
              ...
              >
                  ...
                  <div className="absolute top-3 left-3 px-3 py-1.5 bg-white/90 backdrop-blur-md rounded-full flex items-center gap-1.5 shadow-sm">
                    {getIcon(item.type)}
                    <span className="text-[10px] font-bold text-gray-600 tracking-wide uppercase">{item.type}</span>
                  </div>
            ...
            <div className="p-5">
                {!item.imageUrl && (
                  <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[10px] font-bold text-gray-400 tracking-wide uppercase">{item.type}</span>
                  </div>
                )}

                <h3 className="font-bold text-japan-black text-xl mb-1">{item.title}</h3>
                
                {item.address && (
                    <div className="flex items-start gap-1 mb-4 text-gray-400 text-xs">
                        <MapPin size={12} className="mt-0.5 flex-shrink-0" />
                        <span className="leading-tight line-clamp-1">{item.address}</span>
                    </div>
                )}
                ...
                {item.guideStory && (
                    <div className="mb-4 bg-paper/50 p-4 rounded-2xl border border-sand/50">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs font-bold text-matcha-dark">旅人攻略</span>
                        </div>
                ...
                {!item.guideStory && item.description && (
                  <p className="text-gray-600 text-sm mb-4 leading-relaxed font-medium">{item.description}</p>
                )}
                ...
                {item.highlights && item.highlights.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-5">
                    {item.highlights.map((tag, idx) => (
                      <span key={idx} className={`text-[10px] font-bold px-2.5 py-1 rounded-lg border flex items-center gap-1 ${getHighlightColor(tag.category)}`}>
                        <span className="opacity-60">#</span>{tag.text}
                      </span>
                    ))}
                  </div>
                )}
                ...
                <div className="flex items-center justify-between mt-2 pt-3 border-t border-gray-50">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1.5 bg-japan-black text-white px-5 py-2.5 rounded-2xl text-xs font-bold shadow-md hover:bg-gray-800 active:scale-95 transition-all"
                  >
                    <Navigation size={14} />
                    導航
                  </button>
                ...
                {aiTip && (
                  <div className="mt-4 p-4 bg-white rounded-2xl text-xs text-gray-600 border border-indigo-100 shadow-sm animate-fadeScale relative">
                    ...
                    <div className="flex gap-2">
                        <Sparkles size={16} className="text-indigo-japan flex-shrink-0 mt-0.5" />
        ...
        >
          ...
          <div className="relative w-full h-full flex items-center justify-center flex-col">
            ...
            <div className="mt-6 text-center px-4 animate-fadeScale">
              ...
              {item.location && <p className="text-white/70 text-sm mt-1 flex items-center justify-center gap-1"><MapPin size={12}/> {item.location}</p>}
            </div>
          </div>
        </div>
      )}
